...
<script>
  ...
  let total = <?= json_encode($totalAmount); ?>;
  let order = <?= json_encode($_SESSION['order']); ?>;
  let canEdit = false;
  ...
  // Set quantity from the display and add item to the table
  function setQtyAndAddItem(name, price) {
    ...
    order.forEach((item, index) => {
      if (item.name === name) {
        ...
        order[index].qty = qty;  // Update the quantity
      }
    });
    ...
    updateTable();
    updateTotal(); // Recalculate total after adding item
  }
  ...
  function updateItemQty(index, newQty) {
    ...
    if (newQuantity > 0) {
      ...
      updateTable();
      updateTotal(); // Recalculate total after changing quantity
    }
  }
  ...
  // Prompt for password to edit
  function promptPassword() {
    const passwordModal = new bootstrap.Modal(document.getElementById('passwordModal'));
    passwordModal.show();
  }

  // Validate password to allow editing
